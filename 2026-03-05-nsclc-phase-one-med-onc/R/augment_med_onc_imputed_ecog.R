augment_med_onc_imputed_ecog <- function(
  ...
  med_onc_dat <- med_onc_dat %>%
    mutate(
      md_ecog_exists = !is.na(md_ecog) &
        !(md_ecog %in% "Not documented in note"),
      md_karn_exists = !is.na(md_karnof) &
        !(md_karnof %in% "Not documented in the note")
    )
  ...
  if (add_numeric) {
    med_onc_dat %<>%
      mutate(
        md_ecog_imp_num = suppressWarnings(as.numeric(str_sub(
          md_ecog_imputed,
          0,
          1
        )))
