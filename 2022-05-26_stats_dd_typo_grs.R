...
# parameters
vars <- c("^ca_pros_path_gtert$", "^ca_crc_crm$")
text_fixes <- c("Teriary" = "Tertiary",
                "reater" = "greater")
...
for (var in vars) {
  ...
  for (i in 1:length(text_fixes)) {
    
    text_old <- names(text_fixes)[i]
    text_new <- as.character(text_fixes[i])
    ...
    print(glue("- Number with typo '{text_old}' in choices: {n_contain_typo}"))
  }
}
