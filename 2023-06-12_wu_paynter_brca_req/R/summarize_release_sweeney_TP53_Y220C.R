summarize_release_sweeney_TP53_Y220C <- function(
  id_mut, 
  id_clin_comb,
  id_clin_pt,
  id_clin_sample, 
  ...
  dat_mut %<>%
    filter(hugo_symbol %in% c("TP53")) %>%
    filter(hgvsp_short %in% "p.Y220C") %>%
    # filter(mutation_status %in% "somatic") %>%
    mutate(variant = 1) %>%
    select(-c(hugo_symbol, mutation_status, hgvsp_short))
