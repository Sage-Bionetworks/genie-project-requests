...
Count the number of people who have a metastatic non-small cell lung cancer sample which has an EGFR mutation in the following list:
...
dft_req_table <- dft_mut_sum %>%
    select(minor, hgvsp_short, n_pt) %>%
    tidyr::complete(minor, hgvsp_short, fill = list(n_pt = 0)) %>%
    pivot_wider(
        names_from = minor,
        values_from = n_pt
    ) %>%
...
- Lymph metastases were not included.
- Some of the mutations in the spreadsheet sent did not return any hits (e.g. 729_761del and A763_Y764insFQEA).  Let me know ASAP if that sounds wrong.
