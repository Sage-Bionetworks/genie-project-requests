# Description: Get IDs of all redacted samples from BLADDER and
...
# parameters
cohorts = c("BLADDER")
comments = c("BLADDER 1.1-consortium redacted patients")
...
update_synapse_table <- function(table_id, data, version = NA) {
  
  entity <- synGet(table_id, version = version)
...
create_synapse_table_version <- function(table_id, data, comment = "", append = T, version = NA) {
  
  if (!append) {
    n_rm <- clear_synapse_table(table_id)
  }
  n_add <- update_synapse_table(table_id, data, version = version)
...
for (i in 1:length(cohorts)) {
  ...
  query <- glue("SELECT record_id, cohort, 'redacted' AS reason FROM {synid_table_pat} WHERE cohort = '{cohort}' AND redacted = 'Yes'")
