...
The rates annotated in very small text for each site.  These are from a linear model fit to consoritum releases 9.1 and later.  These are sorted by accrual rate, highest rate sites going first.  All rates are number of participants added per **year**.

```{r, include = T, fig.height = 10}
gg_site_accrual
```

**Observation:** There are clearly some sites, and particularly ones with excellent bioinformatics teams (UHN, JHU), which had a decrease in the number of BRCA cases found over time.  This could be a reflection of improved quality control (for example, reclassifying some mutations that were not truly somatic).
